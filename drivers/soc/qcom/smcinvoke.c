...
/*
 * int_size_add saturates at SIZE_MAX. If integer overflow is detected,
 * this function would return SIZE_MAX otherwise normal a+b is returned.
 */
static inline size_t int_size_add(size_t a, size_t b)
{
	return (b > (SIZE_MAX - a)) ? SIZE_MAX : a + b;
}

/*
 * pad_size is used along with int_size_align to define a buffer overflow
...
/*
 * int_size_align saturates at SIZE_MAX. If integer overflow is detected, this
 * function would return SIZE_MAX otherwise next aligned size is returned.
 */
static inline size_t int_size_align(size_t a, size_t b)
{
	return int_size_add(a, pad_size(a, b));
}
		...
		const union smcinvoke_arg *args_buf)
{
	...
	while (i < SMCI_OBJECT_COUNTS_NUM_BUFFERS(req->counts))
		total_size = int_size_add(total_size,
				int_size_align(args_buf[i++].b.size,
		...
		int32_t *tzhandles_to_release, uint32_t context_type)
{
	...
	*(struct smcinvoke_msg_hdr *)buf = msg_hdr;
	...
	FOR_ARGS(i, req->counts, BI) {
		offset = int_size_align(offset, SMCINVOKE_ARGS_ALIGN_SIZE);
		if ((offset > buf_size) ||
				...
				struct smcinvoke_accept *user_req, int srvr_id)
{
	...
	FOR_ARGS(i, tzcb_req->hdr.counts, BI) {
		user_req_buf_offset = int_size_align(user_req_buf_offset,
				SMCINVOKE_ARGS_ALIGN_SIZE);
